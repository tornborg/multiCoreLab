...
class OriginalRandom {
	...
	public OriginalRandom(int seed)
	{
		w = seed + 1;
		z = seed * seed + seed + 2;
	}
...
class OriginalVertex {
	int			index;
	boolean			listed;
	LinkedList<OriginalVertex>	pred;
	LinkedList<OriginalVertex>	succ;
	BitSet			in;
	BitSet			out;
	BitSet			use;
	BitSet			def;

	OriginalVertex(int i)
	{
		index	= i;
		pred	= new LinkedList<OriginalVertex>();
		succ	= new LinkedList<OriginalVertex>();
		in	= new BitSet();
		out	= new BitSet();
		use	= new BitSet();
		def	= new BitSet();
	...
	void computeIn(LinkedList<OriginalVertex> worklist)
	{
		int			i;
		BitSet			old;
		OriginalVertex			v;
		ListIterator<OriginalVertex>	iter;
...
class OriginalDataflow {
	...
	public static void generateCFG(Vertex vertex[], int maxsucc, OriginalRandom r)
	{
		int	i;
		int	j;
		int	k;
